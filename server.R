...
function(input, output) {
  ...
  output$table <- DT::renderDataTable({
    ...
    datatable(undergrad_table, options = dtoptions, rownames = FALSE) %>%
      formatStyle(input$selected, color = 'darkslategray',
                  background="darkturquoise", fontWeight='bold') %>% #Highlight selected column and bold text
      formatCurrency(c('Average Total Cost', 'Average Tuition', 'Median Debt', 'Median Earnings'), digits = 0) %>% 
      formatPercentage(c('Admission Rate', 'Students With Loans', 'Comp Sci Majors', 'Math Majors'), 1) #Format numbers as % or $ where appropriate
  })  
  ...
  output$densityPlot <- renderPlotly({
    ...
    school1 <- density_data[which(density_data$school_type == "1"),] #Create variables to group by school type
    density1 <- density(school1[[input$density]], na.rm = TRUE) #Determine density plot x-value based on user input
  ...
  output$maxBox <- renderInfoBox({
    ...
    infoBox(paste("Highest", input$mapval, sep = " "), max_state, dollar(max_value), color = "green", icon = icon("money"))
  })
  ...
  output$mymap = renderLeaflet({ 
    palette <- colorBin(c('#fee0d2', #Use colorBin function to choose appropriate bins based on user input variable
                          '#fcbba1',
                          '#fc9272',
                          '#fb6a4a',
                          '#ef3b2c',
  ...
  scatter_data[["Public.style"]] = 'stroke-color: #090F8E'
  ...
  scatter_data[["Private.style"]] = 'stroke-color: #FA0905'
  ...
  scatter_data[["For-Profit.style"]] = 'stroke-color: #FA8B05'
  ...
  output$scatterPlot = renderGvis({  #Create scatterplot with cost and earnings for each college based on user filters
    filtered_scatter2 = filtered_scatter() %>% select(., -1, -2, -3, -4)
    my_options <- list(width="1100px", height="500px",
                       title="Comparing College Cost & Earnings",
                       hAxis="{title:'Median 10-Year Earnings', format: '$#,###'}", #Format axes as dollar amounts
                       vAxis="{title:'Average Cost per Year', format: '$#,###'}",
                       dataOpacity = 0.6)
